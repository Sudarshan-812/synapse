...
import { Database, FileSearch, BrainCircuit, Zap, ChevronDown, MessageSquare, ShieldCheck, Layers } from "lucide-react";

const featuresMenu = [
  {
    icon: Database,
    title: "Enterprise RAG Pipeline",
    description: "768-dim embeddings in pgvector via Supabase",
    href: "#features",
  },
  {
    icon: FileSearch,
    title: "Hybrid Search",
    description: "Vector cosine + BM25 keyword fusion",
    href: "#features",
  },
  {
    icon: BrainCircuit,
    title: "AI Re-ranking",
    description: "Gemini-powered relevance scoring on top chunks",
    href: "#features",
  },
  {
    icon: Zap,
    title: "Zero-Latency Streaming",
    description: "Server-Sent Events for instant token delivery",
    href: "#features",
  },
  {
    icon: MessageSquare,
    title: "Cited Answers",
    description: "Every response backed by exact source citations",
    href: "#features",
  },
  {
    icon: ShieldCheck,
    title: "Workspace Isolation",
    description: "Fully private, tenant-isolated document stores",
    href: "#features",
  },
];
...
const useCasesMenu = [
  {
    ...
    description: "Query large document libraries instantly",
    href: "#how-it-works",
  },
  {
    icon: ShieldCheck,
    title: "Legal & Compliance",
    description: "Find clauses and policies across contracts",
    href: "#how-it-works",
  },
  {
    icon: MessageSquare,
    title: "Internal Knowledge Base",
    description: "Turn company docs into a smart assistant",
    href: "#how-it-works",
  },
];
    ...
    <header className="absolute top-0 inset-x-0 z-50">
      <div className="max-w-[1200px] mx-auto px-6 h-24 flex items-center justify-between">
        ...
        <nav ref={navRef} className="hidden md:flex items-center gap-0.5 relative">
          ...
          <Link
            href="#"
            className="px-4 py-2 text-[14px] font-medium text-zinc-500 hover:text-zinc-950 hover:bg-white/60 rounded-xl transition-all"
          >
            Product
          </Link>
          ...
          <div className="relative">
            <button
              ...
              className={`flex items-center gap-1.5 px-4 py-2 text-[14px] font-medium rounded-xl transition-all ${
                open === "features"
            ...
            >
              Features
              <ChevronDown
                className={`size-3.5 transition-transform duration-200 ${open === "features" ? "rotate-180" : ""}`}
              />
            </button>
              ...
              <div className="absolute top-[calc(100%+10px)] left-1/2 -translate-x-1/2 w-[380px] bg-white/80 backdrop-blur-2xl border border-zinc-200/60 rounded-2xl shadow-[0_24px_60px_rgba(0,0,0,0.12)] p-2 z-50 animate-in fade-in zoom-in-95 duration-150 origin-top">
                  ...
                  {featuresMenu.map((item) => {
                    ...
                    return (
                      <Link
                        ...
                        className="flex items-start gap-3 p-3 rounded-xl hover:bg-zinc-50/80 transition-colors group"
                      >
                ...
                <div className="border-t border-zinc-100 mt-2 pt-2 px-1">
                  <Link
                    href="#features"
                    onClick={() => setOpen(null)}
                    className="flex items-center justify-center gap-1.5 py-2 text-[12.5px] font-semibold text-zinc-500 hover:text-zinc-900 transition-colors"
                  >
                    View all features →
                  </Link>
                </div>
          ...
          <div className="relative">
            ...
            >
              Use Cases
              <ChevronDown
                className={`size-3.5 transition-transform duration-200 ${open === "usecases" ? "rotate-180" : ""}`}
              />
            </button>
